...
| last_login_ip | VARCHAR(64) | NULL | NULL | 最后登录IP |
...
|--------|----------|----------|--------|------|
| id | BIGINT | NOT NULL | AUTO_INCREMENT | 主键ID |
| parent_id | BIGINT | NULL | 0 | 父级机构ID，0表示顶级 |
...
| status | TINYINT(1) | NOT NULL | 1 | 项目状态：0=已归档，1=进行中 |
...
| category_code | VARCHAR(64) | NULL | NULL | 分类编码 |
...
| template_code | VARCHAR(64) | NOT NULL | — | 模型编码（唯一标识） |
| description | TEXT | NULL | NULL | 模型介绍信息 |
| icon | VARCHAR(512) | NULL | NULL | 模型图标URL |
| model_type | VARCHAR(32) | NULL | NULL | 模型类型标签 |
...
| 索引名 | 索引类型 | 字段 | 说明 |
|--------|----------|------|------|
| uk_template_code | UNIQUE | template_code | 模型编码唯一索引 |
...
### 4.6 用户模型参数值表（biz_user_model_param_value）
...
| id | BIGINT | NOT NULL | AUTO_INCREMENT | 主键ID |
| project_id | BIGINT | NOT NULL | — | 所属项目ID |
| user_model_id | BIGINT | NOT NULL | — | 关联用户模型ID |
...
| line_style | VARCHAR(32) | NULL | 'straight' | 连线样式：straight/curve/polyline |
| line_color | VARCHAR(16) | NULL | '#333333' | 连线颜色 |
| is_deleted | TINYINT(1) | NOT NULL | 0 | 逻辑删除标志 |
| create_time | DATETIME | NOT NULL | CURRENT_TIMESTAMP | 创建时间 |
| update_time | DATETIME | NOT NULL | CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP | 更新时间 |
...
|--------|----------|----------|--------|------|
| id | BIGINT | NOT NULL | AUTO_INCREMENT | 主键ID |
| canvas_id | BIGINT | NOT NULL | — | 关联画布ID |
...
| project_id | BIGINT | NOT NULL | — | 关联项目ID |
| canvas_id | BIGINT | NOT NULL | — | 关联画布ID |
| config_id | BIGINT | NOT NULL | — | 关联仿真配置ID |
...
| 索引名 | 索引类型 | 字段 | 说明 |
|--------|----------|------|------|
| uk_task_no | UNIQUE | task_no | 任务编号唯一索引 |
| idx_project_id | NORMAL | project_id | 项目查询索引 |
| idx_canvas_id | NORMAL | canvas_id | 画布查询索引 |
...
| id | BIGINT | NOT NULL | AUTO_INCREMENT | 主键ID |
| task_id | BIGINT | NOT NULL | — | 关联仿真任务ID |
| component_id | BIGINT | NULL | NULL | 关联组件实例ID |
...
| data_content | LONGTEXT | NULL | NULL | 数据内容（JSON格式存储时序数据等） |
| data_file_url | VARCHAR(512) | NULL | NULL | 大规模数据文件存储路径 |
...
sys_user (1) ──── (N) sys_organization        用户归属组织（多对一）
...
biz_canvas (1) ──── (N) biz_simulation_config  画布关联多个仿真配置
biz_simulation_config (1) ── (N) biz_simulation_task  配置对应多次仿真执行
...
CREATE TABLE `biz_project` (
  ...
  `status` TINYINT(1) NOT NULL DEFAULT 1 COMMENT '项目状态：0=已归档，1=进行中',
...
CREATE TABLE `biz_model_template` (
  ...
  `template_code` VARCHAR(64) NOT NULL COMMENT '模型编码',
  `description` TEXT DEFAULT NULL COMMENT '模型介绍信息',
  `icon` VARCHAR(512) DEFAULT NULL COMMENT '模型图标URL',
  `model_type` VARCHAR(32) DEFAULT NULL COMMENT '模型类型标签',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='用户模型参数值表';

-- 3.7 画布表
CREATE TABLE `biz_canvas` (
  `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `project_id` BIGINT NOT NULL COMMENT '所属项目ID',
  `user_model_id` BIGINT NOT NULL COMMENT '关联用户模型ID',
...
CREATE TABLE `biz_canvas_connection` (
  ...
  `line_style` VARCHAR(32) DEFAULT 'straight' COMMENT '连线样式',
  `line_color` VARCHAR(16) DEFAULT '#333333' COMMENT '连线颜色',
  `is_deleted` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '逻辑删除',
  `create_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE `biz_simulation_task` (
  ...
  `project_id` BIGINT NOT NULL COMMENT '关联项目ID',
  `canvas_id` BIGINT NOT NULL COMMENT '关联画布ID',
  `config_id` BIGINT NOT NULL COMMENT '关联仿真配置ID',
...
CREATE TABLE `biz_simulation_result` (
  ...
  `data_content` LONGTEXT DEFAULT NULL COMMENT '数据内容（JSON格式）',
  `data_file_url` VARCHAR(512) DEFAULT NULL COMMENT '数据文件存储路径',
...
### 14.2 登录操作类型（sys_login_log.operation_type）
...
### 14.3 登录状态（sys_login_log.login_status）
...
### 14.5 仿真任务状态（biz_simulation_task.task_status）
...
### 14.6 模型参数类型（biz_model_template_param.param_type）
...
### 14.7 接口方向（biz_model_template_port.port_direction）
...
### 14.8 资源类型（biz_share_record.resource_type）
...
### 14.9 共享目标类型（biz_share_record.share_type）
...
### 14.12 后处理曲线线型（biz_postprocess_curve.line_style）
...
### 14.13 仿真结果数据类型（biz_simulation_result.data_type）
...
| 版本 | 日期 | 修改内容 | 作者 |
|------|------|----------|------|
| V1.0 | 2026-03-04 | 初始版本，包含全部30张业务数据表设计 | — |
